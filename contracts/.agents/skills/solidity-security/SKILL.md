...
describe("Security Tests", function () {
    it("Should prevent reentrancy attack", async function () {
        ...
        const bank = await hre.viem.deployContract("SecureBank");
        const attackerContract = await hre.viem.deployContract("ReentrancyAttacker", [
            bank.address,
        ]);

        // Deposit funds
        await bank.write.deposit({ value: parseEther("10") });

        // Attempt reentrancy attack
        await expect(attackerContract.write.attack({ value: parseEther("1") })).to.be.rejectedWith(
            "ReentrancyGuard: reentrant call",
        );
    });

    it("Should prevent integer overflow", async function () {
        const [owner, attackerAccount] = await hre.viem.getWalletClients();

        const token = await hre.viem.deployContract("SecureToken");

        // Attempt overflow with max uint256
        await expect(token.write.transfer([attackerAccount.account.address, maxUint256])).to.be
            .rejected;
    });
    ...
    it("Should enforce access control", async function () {
        ...
        const contract = await hre.viem.deployContract("SecureContract");

        // Attempt unauthorized withdrawal
        await expect(
            contract.write.withdraw([100n], { account: attacker.account }),
